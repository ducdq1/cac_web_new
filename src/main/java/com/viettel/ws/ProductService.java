...
public class ProductService {
	public static String KiotViet_accesstoken;

	@GET
	@Path("/kiot-viet/token")
	...
	public ProductResponse searchProduct(final SearchProductBO searchProductBO) {
		...
		HangHoaBO tonKho = layThongTinTonKho_KiotViet(maVT);// layThongTinTonKho(maVT);
		if (tonKho != null) {
				...
				for (QuotationDetail quotationDetailNew : createQuotationBO.getLstQuotationDetail()) {
					for (QuotationDetail quotationDetail : quotationDetails) {
						if (quotationDetailNew.getProductCode().equals(quotationDetail.getProductCode())) {
							QuotationResponse quotationResponse = new QuotationResponse();
							quotationResponse.setMessage(String.format("Sản phẩm %s đã được báo giá BG: %s",
									quotationDetailNew.getProductCode(), quotation.getQuotationNumber() == null
											? "(Báo giá mới chưa có mã)" : quotation.getQuotationNumber()));
							quotationResponse.setStatusCode(1000);
							return quotationResponse;
						}
					}
	...
	public  HangHoaBO layThongTinTonKho_KiotViet(String maVT) {
		...
		if (KiotViet_accesstoken == null) {
			KiotViet_accesstoken = layToken();
		}
		...
		try {
			...
			request.setConfig(requestConfig.build());
			request.addHeader("Authorization", "Bearer " + KiotViet_accesstoken);
	...
	public  String layToken() {
		System.out.println("KiotViet_curren_accesstoken: " + KiotViet_accesstoken);
		CloseableHttpResponse response = null;
