...
public class GenericDAOHibernate<T, ID extends Serializable> extends GenericHibernateDAO<T, ID> {
	...
	// char[] lowerVietnameseChar = {'à', 'á', 'ạ', 'ả', 'â', 'ầ', 'ấ', 'ẩ',
	// 'ậ', 'ă', 'ằ', 'ắ', 'ặ', 'ẳ', 'đ', 'è', 'é', 'ẹ', 'ẻ', 'ê', 'ề', 'ế',
	// 'ệ', 'ể', 'ì', 'í', 'ị', 'ỉ',
	// 'ò', 'ó', 'ọ', 'ỏ', 'ô', 'ồ', 'ố', 'ộ', 'ổ', 'ù', 'ú', 'ụ', 'ủ', 'ư',
	// 'ừ', 'ứ', 'ự', 'ử'};
	// char[] upperVietnameseChar = {'À', 'Á', 'Ạ', 'Ả', 'Â', 'Ầ', 'Ấ', 'Ẩ',
	// 'Ậ', 'Ă', 'Ằ', 'Ắ', 'Ặ', 'Ẳ', 'Đ', 'È', 'É', 'Ẹ', 'Ẻ', 'Ê', 'Ề', 'Ế',
	// 'Ệ', 'Ể', 'Ì', 'Í', 'Ị', 'Ỉ',
	// 'Ò', 'Ó', 'Ọ', 'Ỏ', 'Ô', 'Ồ', 'Ố', 'Ộ', 'Ổ', 'Ù', 'Ú', 'Ụ', 'Ủ', 'Ư',
	// 'Ừ', 'Ứ', 'Ự', 'Ử'};
	// char[] toChar = {'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a', 'a',
	// 'a', 'a', 'a', 'd', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'i',
	// 'i', 'i', 'i',
	...
	// }
	//
	// public String convertToSearchVietNamese(String columnName, String source,
	// List lstParam) {
	// if (source == null) {
	// return source;
	// }
	...
	// public List<T> getDataByFields(Class entityClass, List<String> key,
	// List<Object> valueOfKey, String sortField) {
	// String entityName = entityClass.getSimpleName();
	// // Build sql
	// StringBuilder sqlBuilder = new StringBuilder();
	// sqlBuilder.append(" FROM ").append(entityName).append(" obj WHERE ");
	// if (key != null && valueOfKey != null && key.size() > 0 && (key.size() ==
	// valueOfKey.size())) {
	// sqlBuilder.append("obj.").append(key.get(0)).append("= :value0");
	// for (int i = 1; i < key.size(); i++) {
	// sqlBuilder.append(" AND ").append("obj.").append(key.get(i)).append("=
	// :value").append(i);
	// }
	// }
	// if (!"".equals(sortField)) {
	...
	// StringBuilder sqlBuilder = new StringBuilder();
	// sqlBuilder.append(" FROM ").append(entityName).append(" obj WHERE
	// obj.").append(entityField).append(" IN (:key)");
	// Query query =
	// session.createQuery(sqlBuilder.toString()).setParameterList("key", key);
	// return query.list();
	// }
	 ...
	 * @return
	 */
	// public Boolean checkEntityExistedForUpdate(String idField, String
	// codeField, String entityCode, Long entityId) {
	 ...
	 * @return
	 */
	// public Boolean checkEntityExistedForInsert(String codeField, String
	// entityCode) {
	// Boolean isExisted = false;
	// try {
	//
	//// Criterion[] criterion = {Restrictions.like(codeField,
	// entityCode.toLowerCase(),MatchMode.EXACT).ignoreCase()};// new
	// Criterion[1];
	 ...
	 * @return
	 */
	// public Boolean isExistIDInDb(Class entityClass, List<String> key,
	// List<Object> valueOfKey) {
	// String entityName = entityClass.getSimpleName();
	// // Build sql
	// StringBuilder sqlBuilder = new StringBuilder();
	// sqlBuilder.append(" FROM ").append(entityName).append(" obj WHERE ");
	...
	// String enityUpdate, Object valueUpdate) {
	// String entityName = type.getSimpleName();
	// // Build sql
	// StringBuilder sqlBuilder = new StringBuilder();
	// sqlBuilder.append(" FROM ").append(entityName).append(" obj WHERE ");
	...
	// {
	// sqlBuilder.append(" and ").append("obj.").append(enityUpdate).append(" <>
	// :").append(enityUpdate);
	// }
	// }
	// Query query = session.createQuery(sqlBuilder.toString());
	...
	// sqlBuilder.append(" FROM ").append(entityName).append(" obj WHERE
	// obj.isActive = 1 and lower(obj.name) like ? escape'!'");
	...
	// // Build sql
	// if (id != null && id > 0L) {
	// String hql = "from " + entityName + " where " + idName + " = " +
	// id.toString();
	...
	// public String convertToJSONArray(List<T> array, String id, String
	// searchAttr1, String searchAttr2) {
	...
	/**
	 * Split chuoi 1;2 hoac 1;2; thanh mang long [1,2] Split chuoi 1; thanh mang
	 * long [1]
	 * 
	 * @param input
	 * @param delimiter
	...
	/**
	 * Split chuoi 1;2 hoac 1;2; thanh mang chuoi ['1', '2'] Split chuoi 1;
	 * thanh mang chuoi ['1']
	 * 
	 * @param input
	 * @param delimiter
	...
	public void deactiveRecord(String table, Long fileId) {
		Query query = getSession().createQuery("UPDATE " + table + " set isActive=0 where isActive =1 AND fileId=?");
