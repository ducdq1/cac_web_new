...
public class DanhSachThoThay extends BaseComposer {
	...
	private List<Workers> lstProducts;
	...
	private void reloadModel(PhamarcyFileModel searchModel) {
		try {
			...
			PagingListModel plm = dao.findCustomers(tbTenKH.getText().trim(),start,take);
	...
	public void onDeleteSP(Event event) {
		final Workers obj = (Workers) event.getData();
		String message = "Bạn muốn xóa khách hàng này?";
		EventListener<ClickEvent> clickListener = new EventListener<Messagebox.ClickEvent>() {
			...
			public void onEvent(ClickEvent event) throws Exception {
				...
				if (Messagebox.Button.YES.equals(event.getButton())) {
					obj.setIsActive(0L);
					new WorkerDao().saveOrUpdate(obj);
