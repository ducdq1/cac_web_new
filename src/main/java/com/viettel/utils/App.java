...
//    }
    ...
    public static P createParagraphWithFontSize(ObjectFactory factory, String content, int fontSize) {
        P paragraph = factory.createP();
        String n;
        String sup ;
        String sub ;
        ...
        while (i < content.length() - 1) {
            if (content.substring(nindex).contains("_(") == false && content.substring(nindex).contains("^(") == false) {
                paragraph.getContent().add(createNormalTextWithFontSize(factory, content.substring(nindex), fontSize));
                break;
            } else {
                //kiem tra chuoi co chua ki tu danh dau k
                if (content.substring(nindex).contains("^(")) {
                    ...
                    if (content.charAt(i) == '^' && content.charAt(i + 1) == '(') {
                        n = content.substring(nindex, i);
                        paragraph.getContent().add(createNormalTextWithFontSize(factory, n, fontSize));
                        //n = "";
                        ib = i;
                        supindex = ib + 2;
                        for (int j = supindex; j < content.length(); j++) {
    ...
    private static R createNormalTextWithFontSize(ObjectFactory factory, String message, int fontSize) {
        Text normalText = factory.createText();
        normalText.setValue(message);
        normalText.setSpace("preserve");
        R normalRun = factory.createR();
        ...
        setFontSize(normalRunProperties, size);
        normalRun.setRPr(normalRunProperties);
        return normalRun;
    }
    ...
    public static R createRWithFontSize(ObjectFactory factory, String content, int fontSize) {
        R paragraph = factory.createR();
        String n ;
        String sup ;
        String sub;
